...
function copy(lang) {
  if (lang === "en") {
    return {
      ...
      dimensionsHint: "Bars show your score per area as a percentage.",
  ...
  return {
    ...
    dimensionsHint: "Die Balken zeigen Ihre Ausprägung pro Bereich in Prozent.",
  ...
  const dimRows = dimensions
    .map(function (d) {
      ...
      return (
        ...
        `<td style="padding:10px 0;border-bottom:1px solid #e8edf2;color:#5a7184;font-size:14px;text-align:right;white-space:nowrap;">${escapeHtml(pct)} %</td>` +
          ...
          <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="max-width:560px;background:#ffffff;border:1px solid #e8edf2;border-radius:16px;overflow:hidden;">
            ...
            <tr>
              <td style="padding:28px;">
                ...
                <div style="font-size:16px;font-weight:700;margin-bottom:6px;color:#0a1f33;">${t.dimensionsTitle}</div>
                <div style="font-size:13px;line-height:1.5;color:#5a7184;margin-bottom:12px;">${t.dimensionsHint}</div>
...
function buildResultText(data, siteUrl, lang) {
  ...
  dimensions.forEach(function (d) {
    lines.push(`${d.name}: ${Number(d.pct) || 0} %`);
  });
